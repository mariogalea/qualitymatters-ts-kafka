...
export class KafkaProducer {
    ...
    constructor(broker: string, topic: string) {
      ...
      try {
        ...
        this.topic = topic;
        Logger.debug(`OrderService KafkaProducer initialized for topic '${this.topic}' with broker '${broker}'`);
      } catch (error) {
        Logger.error('Failed to initialize OrderService KafkaProducer:', error);
        throw error;
      }
    }
